cc.Class({
  ...
  terriAreaFill: function (gridX, gridY) {
    if (this.walkPath.length > 2) {
      let node = new Object();
      let start = new Object();
      start.x = gridX;
      start.y = gridY;
      let dest = new Object();
      dest.x = this.walkPath[0].x;
      dest.y = this.walkPath[0].y;
      node = this.BFS(start, dest);
      let map = this.map;
      cc.log('传回的map');
      cc.log(dest);
      cc.log(map);
      ...
      if (node !== null) {
        ...
        cc.log(gridX, gridY);
        // debug info
        // for (i = 0; i != this.walkPath.length; i++) {
        //   this.dyeGrid(
        //     this.walkPath[i].x,
        //     this.walkPath[i].y,
        //     this.TERRITORYCOLOR,
